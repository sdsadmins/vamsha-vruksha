...
export default function IdentityPage() {
  ...
  return (
    <div className="min-h-screen" style={{background:"linear-gradient(160deg, #FAF7F2, #F0E6D3)"}}>
      {/* Top nav */}
      <div className="border-b bg-white/80 backdrop-blur-sm px-6 py-4 flex items-center justify-between" style={{borderColor:"#E8D5BC"}}>
        <Link href="/" className="flex items-center gap-2">
          ...
          <span className="font-bold text-sm" style={{fontFamily:"'Playfair Display', serif", color:"#1B4332"}}>Vamsha Vruksha</span>
        </Link>
          ...
          {[{n:1,label:"Identity",active:true},{n:2,label:"Lineage",active:false},{n:3,label:"Heritage",active:false}].map(s => (
            <div key={s.n} className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all"
                style={s.active ? {background:"linear-gradient(135deg,#1B4332,#2D6A4F)",color:"white"} : {background:"#E8D5BC",color:"#9CA3AF"}}>
                {s.n}
              </div>
              <span className="text-xs font-medium hidden sm:block" style={{color:s.active?"#1B4332":"#9CA3AF"}}>{s.label}</span>
              {s.n < 3 && <div className="w-8 h-0.5 mx-1" style={{background:s.active?"#1B4332":"#E8D5BC"}} />}
            </div>
          ))}
        </div>
        <div className="w-24" />
      ...
      <div className="max-w-5xl mx-auto px-6 py-12">
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.5}}>
          <p className="text-sm font-semibold mb-2" style={{color:"#A67C52", letterSpacing:"0.1em"}}>STEP 1 OF 3</p>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-5">
              ...
              <div className="rounded-2xl border p-6 space-y-5" style={{background:"white", borderColor:"#E8D5BC"}}>
                <div>
                  <label className="block text-sm font-semibold mb-2" style={{color:"#1B4332"}}>ID Type</label>
                  <select className="w-full px-4 py-3 text-sm border rounded-xl outline-none" style={{borderColor:"#E8D5BC", background:"white"}}>
                ...
                <div>
                  ...
                  <motion.div onClick={() => setUploaded(true)} whileHover={{scale:1.01}} whileTap={{scale:0.99}}
                    ...
                    style={{borderColor: uploaded ? "#1B4332" : "#E8D5BC", background: uploaded ? "#F0FBF4" : "white"}}>
                    {uploaded ? (
                      <div>
                      ...
                      <div>
                        ...
                        <button className="mt-4 px-5 py-2 text-sm font-semibold text-white rounded-xl" style={{background:"linear-gradient(135deg, #A67C52, #D4AF7A)"}}>Select File</button>
                      </div>
                    )}
                  </motion.div>
                </div>
                ...
                <div>
                  ...
                  <motion.div onClick={() => setSelfie(true)} whileHover={{scale:1.01}}
                    ...
                    style={{borderColor: selfie ? "#1B4332" : "#E8D5BC", background: selfie ? "#F0FBF4" : "white"}}>
            ...
            <div className="space-y-4">
              <div className="rounded-2xl p-5" style={{background:"linear-gradient(160deg, #0D2B1E, #1B4332)"}}>
                <div className="flex items-center gap-2 mb-3">
                  <ShieldCheck size={16} style={{color:"#D4AF7A"}} />
                  <span className="text-sm font-semibold" style={{color:"#D4AF7A"}}>Trust &amp; Security</span>
                </div>
              ...
              <div className="rounded-2xl p-5 border" style={{background:"white", borderColor:"#E8D5BC"}}>
                <p className="text-xs font-semibold mb-2" style={{color:"#A67C52"}}>STEP 1 OF 3</p>
